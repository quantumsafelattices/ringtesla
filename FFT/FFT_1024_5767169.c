...
#include "FFT_1024_5767169_constants.h"
...
*/
void FFT_forward_1024_5767169(FFTSHORT x[1024]) {
        const FFTSHORT n = 1024;
	const FFTSHORT q = 5767169;
	FFTSHORT index, step;
	FFTSHORT i,j,m;
	FFTSHORT t0,t1;
	...
	for (m = n>>1; m >= 1; m=m>>1) {
		...
		for (j = 0 ; j < m; ++j) {
			for (i = j; i < n; i += (m<<1)) {
				...
				ADD(t1, x[i], q - x[i+m]);
				MUL_MOD(x[i+m], t1, W_1024_5767169[index], q);				
				x[i] = t0;				
			}
...
*/
void FFT_backward_1024_5767169(FFTSHORT x[1024]) {
        const FFTSHORT n = 1024;
	const FFTSHORT q = 5767169;
	FFTSHORT index, step;
	FFTSHORT i,j,m;
	...
	for (m = 1; m < n; m=m<<1) {
		...
		for (j = 0 ; j < m; ++j) {
			for (i = j; i < n; i += (m<<1)) {							
				t0 = x[i];
				t0 -= (t0 >= q) ? q : 0;
				MUL_MOD(t1, x[i+m], W_rev_1024_5767169[index], q);				
				ADD(x[i], t0, t1);
				ADD(x[i+m], t0, q - t1);
...
the fourier transform using decimation-in-time.
*/
void FFT_twisted_forward_1024_5767169(FFTSHORT x[1024]) {
        const FFTSHORT n = 1024;
	const FFTSHORT q = 5767169;
	FFTSHORT index, step;
	FFTSHORT i,j,m;
	...
	j = 0;
	for (i = 0; i < n>>1; ++i) {
		MUL_MOD(x[j], x[j], W_1024_5767169[i], q);
		j++;	
		MUL_MOD(x[j], x[j], W_sqrt_1024_5767169[i], q);	
		j++;
	}
...
fourier transform is using decimation-in-frequency.
*/
void FFT_twisted_backward_1024_5767169(FFTSHORT x[1024]) {
        const FFTSHORT n = 1024;
	const FFTSHORT q = 5767169;
 	FFTSHORT index, step;
	FFTSHORT i,j,m;
	...
	j = 0;
	for (i = 0; i < n>>1; ++i) {
		MUL_MOD(x[j], x[j], W_rev_1024_5767169[i], q);
		j++;	
		MUL_MOD(x[j], x[j], W_sqrt_rev_1024_5767169[i], q);
		j++;
	} 
}


void _FFT_forward_1024_5767169(FFTSHORT *x) {
  FFT_twisted_forward_1024_5767169(x);
}

void _FFT_backward_1024_5767169(FFTSHORT *x) {
  FFT_twisted_backward_1024_5767169(x);
  size_t i;
  for (i=0; i<1024; ++i)
    MUL_MOD(x[i], x[i], 5761537, 5767169);
}
